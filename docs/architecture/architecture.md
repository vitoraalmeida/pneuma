...
|---|---|---|
| `src/main.rs` and `src/cli/` | CLI definition (`src/cli/args.rs`), command dispatch and handlers by capability (`src/cli/`), error classification with message and exit-code mapping (`src/cli/error.rs`), result rendering (`src/cli/output.rs`), host configuration, temporary import checkout preparation | Domain decisions or persistence rules |
