...
| INV-DB-007 | The database opens only with the exact current baseline ledger, and backup/restore accept only that schema: restore validates source integrity and the exact current ledger before replacing the live database. A database-wide kernel lock (shared for normal commands, exclusive for restore) serializes replacement against every other database user; `version` stays lock-free. | Persistence invariant | `src/adapters/database.rs` (`open`, `backup`, `restore_and_verify`, `DatabaseLock`); shared-lock acquisition in `src/cli/mod.rs`/`src/cli/ci.rs` | Same (adapter owns lock mechanics and validation; CLI acquires the shared lock around database use) | Empty databases initialize atomically; incompatible schemas and old-generation backups are rejected before any live mutation; process death releases the lock; contention is a conflict | In-file database tests (lock mode serialization, process-death release, incompatible/corrupt backup rejection before replacement, busy restore); `tests/cli.rs::restore_rejects_an_incompatible_backup_without_replacing_the_live_database`, `restore_conflicts_while_normal_access_holds_the_database_lock`, `normal_commands_conflict_while_restore_holds_the_exclusive_lock` | Keep |
