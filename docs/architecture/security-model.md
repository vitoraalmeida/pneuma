...
| Backup substitution | Restore structurally valid but attacker-modified intent and history. | Restore validates SQLite integrity and the exact current schema before replacing the live database, and creates a pre-restore backup. | Integrity and schema checking do not authenticate the backup producer or contents. |
