...
for fixture in "$FIXTURES_DIR"/*/; do
	...
	echo "  -> Importing $name"
	output=$(
		remote_ssh "$SSH_HOST" "runuser -u pneuma -- bash -l -s \"$name\"" <<'REMOTE'
set -euo pipefail
name="$1"
...
for fixture in "$FIXTURES_DIR"/*/; do
	...
	remote_ssh "$SSH_HOST" "runuser -u pneuma -- bash -lc 'cd \$HOME && pneuma app deploy $name --image $REGISTRY/$name@$digest 2>&1'" 2>&1 | grep -v level=warning || true
done
